...
<title>Pesquisar cliente - GET CONSTRUÇÕES</title>
...
<div class="sidebar1">
  ...
  <div class="content">
   <div id='cssmenu'>
    ...
    <p align="center"><b>	LISTAR PRODUTO POR FORNECEDOR</b> </p>
	<center>
    <?php
		...
		$forn->conectar();
		$forn->setCnpj($cnpj);
		$forn->pesquisa_forn_prod($forn->getCnpj());
		$forn->fornecedor_produto($forn->getCnpj());
		echo "<hr/>";
		echo "<center><form action='cadastrarProduto_fornecedor.php' name='frmindex' method='post'><table><tr><td><input type='submit' name='Novo Produto' value='Novo Produto' class='botaoForm2' /> <input type='hidden' value='". $forn->getCnpj() ."' name='fake2'></form></td><td>
		<form action='alterar_excluirFornecedor.php' name='frmindex' method='post' >
					<input type='hidden' value='". $forn->getCnpj() ."' name='fake'><input type='submit' name='alterar' value='Alterar' class='botaoForm2'/>
					</td><td>
        	<input type='submit' name='alterar' value='Excluir' class='botaoForm2'/><form></td></tr></table>
        </center>";
	?>
	</center>
    </div>
